...
class Feistel:
    ...
    def bit_right_2(bits6):
        return bits6[-1] + bits6[-2]

    @staticmethod
    def bit_middle_3(bits6):
        return bits6[-2] + bits6[-3] + bits6[-4]
    ...
    def do_feistel(self, key32_left, key48_round, expansion_matrix, permutation_round_matrix):
        ...
        for bits6 in enumerate(bits6list):
            subtitution_s_matrix = Extract_ConstantesDES.recup_constantes_des().const_des["S" + box_count]
            ...
            middle3 = self.conv_bin_to_decimal(self.bit_middle_3(bits6))
            box_value = self.conv_decimal_to_bin(subtitution_s_matrix[right2][middle3])
